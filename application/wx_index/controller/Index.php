...
class Index extends Controller
{
 ...
 //    );
    ...
    //     $token = session('token');
    //     if($token){
    //         $this->auth = new WechatAuth(Config('appid'), Config('appsecret'), $token);
    //     } else {
	...
	public function index(){
		...
		// //测试数据
		// //try{
		// 	$this->wechat = new Wechat(Config('token'),Config('appid'),Config('crypt'));
		// 	$data = $this->wechat->request();

		// 	// if($data->MsgType == 'text' && $data->Content == '你好'){
		// 	// 	$this->wechat->replyText("你好");
		// 	// }else{
		// 	 	$this->wechat->replyText("听不懂");
		// 	// }

		// //}catch(\Exception $e){
		// 	//file_put_contents('./error.json', json_encode($e->getMessage()));
		// //	$this->wechat->replyText("yichang");
		// //}
	}
